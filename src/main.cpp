...
#if defined(NDEBUG)
# error "Atechcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xec7611f3362750dddf9cf1a2d35296959d1db1bb7dbf3cd6566909c6e1ad8211");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Atechcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Atechcoin Signed Message:\n";
...
{
    // Atechcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Atechcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Atechcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Atechcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Atechcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Atechcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x06554e4293a54d438c9c2e7f0aded1bcb8e8d0dd24d63759c78976afa7f11f80");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is the release Atechcoin dec 2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04299f6b9a64d6613221d6dcf175590142310e0f7afb678408104b8ca1686b846abd8f3201b0aa7342944fd1e8b5a5df509fa13826e63968569350d44abaed9e77") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1545923036;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087981024;

        if (fTestNet)
        {
            block.nTime    = 1545922985;
            block.nNonce   = 386406234;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x97063bf3a095d73c0b7d1bd25d365e22f4f6431a4c0c6e6c449163059d54402c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Atechcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AtechcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AtechcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AtechcoinMiner(CWallet *pwallet)
{
    printf("AtechcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("atechcoin-miner");
    ...
    try { loop {
        ...
        printf("Running AtechcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AtechcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AtechcoinMiner, pwallet));
}
